...
Er wird, im Gegensatz zum Mittelwert (bei metrischen Daten), nicht durch extreme Werte verzerrt [@kaliyadan2019].
...
Der Median wird, im Gegensatz zum Mittelwert, nicht durch extreme Werte (outliers) verzerrt [@crump2018].
...
Achtung: Der Mittelwert wird durch extreme Werte (outliers) verzerrt. 
...
Sie ist die Wurzel aus der Varianz einer Variablen und liegt somit auf demselben Skalenniveau wie der Mittelwert.
...
In theoretisch gut begründeten kann man auch einseitig testen.

Da t-Tests zu den parametrischen Verfahren gehören, müssen Annahmen erfüllt sein, um diese berechnen zu dürfen:

- ausreichend große Stichprobe (Faustregel n=30)
- normalverteilte Daten

Sind die Annahmen verletzt, kann man auf nicht-parametrische Alternativen des jeweiligen t-Tests ausweichen. Die Unterarten des t-Tests haben z.T. zusätzlich zu den oben genannten noch weitere Annahmen, für die bei Verletzung ein Alternativtest am Ende des Abschnitts aufgelistet wird. [@crump2018]

#### one-sample t-Test

Der one-sample t-Test vergleicht einen Stichprobenmittelwert mit einem (geschätzten) Populationsmittelwert, um zu schauen, ob die Stichprobe mit ausreichender Wahrscheinlichkeit aus dieser Population stammt oder aus einer anderen. 
...
Ist der t-Test signifikant, gibt es eine Abweichung zwischen der Stichprobe und der Population, die mit ausreichender Wahrscheinlichkeit nicht zufällig zustandegekommen ist. Ist er nicht signifikant, geht man davon aus, dass die Stichprobe aus der Population stammt.
...
Der correlated-samples t-Test (auch paired-samples t-Test) ist dem one-sample t-Test sehr ähnlich. Er wird häufig für within-subjects Experimente genutzt (z.B. die Gedächtnisleistung einer Person vor einem Training wird mit ihrer Leistung nach einem Training verglichen) oder wenn Versuchpersonen aus zwei Gruppen miteinander verbunden sind (z.B. Zwillinge, Paare etc.).
Die Berechnung ist ähnlich wie beim one-sample t-Test. [@crump2018]

Nicht-parametrische Alternative: Wilcoxon Signed-Rang Test (wenn die Differenzen nicht normalverteilt sind; Variable darf ordinalskaliert sein)

#### independent-samples t-Test

Der independent-samples t-Test wird für between-subjects Experimente (z.B. der Mittelwertsvergleich zweier Gruppen, die verschiedene Treatments bekamen) genutzt, ähnelt aber ansonsten stark den bereits genannten t-Tests. [@crump2018]
...
Man nutzt Varianzanalysen, um herauszufinden, ob gefundene Mittelwertsunterschiede zwischen Gruppen überzufällig sind oder nur durch Messfehler zustande kamen.
...
Einfach mehrere t-Tests zu berechnen, würde das die Wahrscheinlichkeit eines Alpha-Fehlers erhöhen und ist daher keine sinnvolle Alternative.
