...
ggplot(normal, aes(x = x, y = dummy, fill = factor(stat(quantile)))) +
  ...
  geom_vline(xintercept = seq(70, 130, 15), color = "sienna") + 
  scale_fill_manual(values = c("orange", "darkorange", "orangered", "darkorange", "orange")) +
...
ggplot(data, aes(x = X, y = Y)) +
  geom_jitter() +
  geom_smooth(formula = 'y ~x', method = lm, se = FALSE, color = "sienna") +
  geom_vline(xintercept = mean(data$X), color = "orange") +
  geom_hline(yintercept = mean(data$Y), color = "orangered") +
  theme_minimal() +
  theme(axis.title.x = element_text(color = "orange", size = 12),
        axis.title.y = element_text(color = "orangered", size = 12))
```
